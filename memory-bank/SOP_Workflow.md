...
4. 证据链完整：每个 PR 必须在 TechDesign.md 的 Changelog 追加记录；每个 PR 有描述、验收步骤、风险与回滚说明。
...
### 2.4 CLI（Command Line Interface，命令行界面）（验收与证据）
1. 用于：构建/测试/启动/接口调用/查看日志/运行 compose。
2. 输出要求：每条命令必须带注释，说明用途与要获取的信息；每个 PR 至少记录一组“命令 + 预期输出/现象”作为证据。
## 3. 目录改动范围（默认白名单）
1. 允许改：backend/、memory-bank/、.github/、scripts/、docker/（若存在）。
...
6. 必须补充必要文档：PRD/TechDesign/ENV/SOP/QualityGate（按需）。
### 4.2 标准输出结构（模型必须按此输出）
...
1. 必读文档（显式使用）：PRD/TechDesign/ENV/SOP/QualityGate（只读）。
2. 查看仓库结构与现状（不改代码）。
3. 输出：现状摘要 + 风险点 + 改动边界。
### Step 2：计划先行（Plan-first）
1. 输出文件级改动清单（改哪些文件、原因、边界）。
2. 明确影响面：接口/数据库/配置/日志/测试。
3. 输出：验收清单（至少 1 条可复现命令，命令需带注释与预期现象）。
### Step 3：实现（Implement）
1. 优先用 Codex 按清单修改。
2. 过程中遇到局部问题再用 VS Code 聊天窗口。
3. 禁止超范围改动：若发现需要扩展范围，必须回到 Step 2 更新计划。
### Step 4：验收（Verify）
1. 用 CLI 按 CHECKLIST_QualityGate.md 执行验收项。
2. 收集证据：命令、关键输出、Swagger/日志现象。
3. 若失败：记录失败现象与下一步，不合并。
### Step 5：记录与提交（Document & Submit）
1. 在 TechDesign.md 的 Changelog 追加：本 PR Done/Verify/Result（只写事实）。
2. 提交前自检：变更是否符合 PR 主题；是否遗漏测试/文档同步。
3. 在 GitHub 发 PR：填写变更点、验收方式、风险与回滚。
## 6. 质量门禁（每个 PR 必须满足）
1. 能构建（Build）：至少不破坏现有构建流程。
2. 能运行（Run）：至少能启动或能被 Swagger/MockMvc 验证。
3. 能验证（Verify）：提供命令 + 预期结果（可复现）。
4. 有记录（Record）：TechDesign.md 的 Changelog 有对应条目。
5. 有边界（Scope）：一次 PR 一个主题，目录白名单不越界。
## 7. 文档同步规则（触发式维护）
1. ENV.md：记录环境与版本（安装/升级工具后必须更新）。
2. TechDesign.md：记录技术方案与模块边界；每个 PR 在 Changelog 追加记录。
3. PRD.md：需求范围/里程碑变化时更新。
4. SOP_Workflow.md：流程规则变化时更新。
5. CHECKLIST_QualityGate.md：门禁项变化时更新。

## PR 流程补充（以 PR-001 为样例）
- 定位：CI/CD（Continuous Integration/Continuous Delivery/Deployment，持续集成/持续交付/持续部署）是成熟的工程化流程；本项目先落地 CI 作为质量门禁。
- 固定步骤（每个 PR 都按这个走）：
  1) 只读摸底（显式阅读 PRD/TechDesign/ENV/SOP/QualityGate），输出改动计划与验收点
  2) 实现改动（限制改动范围，避免无关文件漂移）
  3) 本地验收（按 CHECKLIST_QualityGate 清单逐项验证）
  4) 写入记录（在 TechDesign.md 的 Changelog 追加 Done/Verify/Result）
