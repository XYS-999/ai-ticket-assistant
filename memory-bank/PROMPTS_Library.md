...
## 教材式讲解（按功能实现顺序 + 文件创建顺序）
〖目标〗把 AI 生成过程变成可学习、可复现的“手工实现教程”，最终达到：不依赖 GPT 也能自己从零搭建；能应对面试追问
〖讲解顺序〗按“实现一个功能”的自然顺序讲解（例如：Maven → Maven Wrapper → Spring Boot 入口 → Controller → Test → CI），同时遵循“文件创建顺序”
〖节奏控制〗每次只讲 1 个功能或 1 个文件：先讲概览（1段），再讲语法/配置细节（逐行/逐段），最后停下来等我提问；我问完再进入下一步
〖每步必须包含〗目的｜涉及技术栈｜修改/新增了哪些文件｜核心语法/参数说明｜如何验证（只给关键入口/命令）｜可能的面试追问点（可选）
〖限制〗不要一次讲完全部文件；不要跳步；不确定就先说明假设并给验证方式
〖输出格式〗
- 本步目标：
- 涉及技术栈：
- 文件与作用：
